...
**UI layer: GPUI** sourced from `zed-industries/zed`, not the crates.io `gpui` v0.2.x (which is a name-squat). GPUI is the only Rust framework that produces 120Hz ProMotion-native output on macOS today. Tauri's WKWebView has a hard 60 FPS ceiling. egui/Iced are not Mac-native. **Pinning strategy (Phase 0 spike S2):** in `[workspace.dependencies]`, declare `gpui = { git = "https://github.com/zed-industries/zed" }` **without** a `rev` — it dedupes with gpui-component's transitive resolution, producing a single Zed clone instead of two. Reproducibility comes from committed `Cargo.lock`; bump gpui by editing the lock deliberately, never via `cargo update`. Cost: weekly breaking changes absorbed when we bump the lock. **Construction (Week 2 finding):** `gpui::Application::new()` does not exist at HEAD — use `gpui_platform::application()` (one-liner returning `Application` with the default macOS platform); `gpui_platform` is declared alongside `gpui` in `[workspace.dependencies]`. **Activation-policy timing:** set `NSApplication.setActivationPolicy(Accessory)` **inside** the GPUI `run` closure, not before; calling `sharedApplication` before GPUI installs `APP_CLASS` gets a plain NSApplication whose `platform` ivar later explodes. **Dev-host prereq:** `xcodebuild -downloadComponent MetalToolchain` once per machine (Xcode 26+ builds GPUI's Metal shaders; ~700MB) — see PHASE_1_PLAN.md §1.5. **Widgets: `longbridge/gpui-component`** — the shadcn/ui of GPUI, Apache-2.0 — pinned to `rev = "808df4069295ef4e0b3b01445671c3c7057377cd"` (2026-04-23 main HEAD, compile-verified by S2+S4); covers buttons/inputs/code editor/markdown/dock/charts.
