...
- File search uses Spotlight first and a compact fallback filename catalog. Settings > Files has VISH-level folder scoping on top of macOS Full Disk Access: default toggles for Desktop, Documents, Downloads, Pictures, Movies, Music, User Apps, Shared, and External Volumes plus custom excluded folders. Exclusions apply to Spotlight results, fallback indexing, semantic/vector candidates, and FSEvents maintenance.
- File fallback maintenance is event-driven through FSEvents: 2s stream latency, 2s app debounce, persistent event IDs, and no periodic full rescan unless FSEvents reports dropped/coalesced/root-change events or folder scope changes require a warm rebuild.
...
- Clipboard history is opt-in, text-only, and trigger-based through `clip` / `clipboard`; it does not run in default search. Clipboard v2 includes pinned entries, retention pruning, source-app metadata, per-source app exclusions, Settings edit/delete/clear controls, and Tab actions for Edit & Paste, Pin / Unpin, and Delete.
- The clipboard monitor polls `NSPasteboard.changeCount` only while enabled, stores at most 100 text entries, skips entries above 50k characters, prunes unpinned entries by the selected retention period, and persists to `Application Support/vish/clipboard.plist`.
...
- Settings uses a minimal SwiftUI control-deck layout with a left section rail, neutral graphite background, iOS-style glass switches, and a single iOS-blue accent palette; avoid explanatory text density and keep it out of the launcher hot path. The default pane is Setup, which shows compact readiness cards for Launcher, Files, Clipboard, AI, and Privacy plus the top production performance budgets. Privacy is Settings-only and summarizes Accessibility, Full Disk visibility, Clipboard retention/counts, Local AI status, and local Application Support storage.
